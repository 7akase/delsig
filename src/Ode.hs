...
type XdotV = Double          -- time
             -> Vector Double   -- initial condition
             -> Vector Double   -- differential equation 


sim :: XdotV -> VecEle -> VecEle
-- | simulate xdotV for single timeslot
sim xdotV init_cond = 
  last . toRows $ odeSolveV RKf45 
    ...
    where
      ...
      abstol = 1e-3
      reltol = 1e-6
      init_cond' = eventAtAlpha init_cond
      ts = vector [0, 1.0 / fs]
...
getLog :: [VecEle]
getLog = iterate (sim (xdotV u')) (vector (replicate 9 0))
...
sample3 = do
  ...
  mplot . fmap vector . transpose $ 
    zipWith (:) (fmap fromIntegral [0 .. n_fft - 1])
                (pick [0,1,2] . fmap toList . take n_fft . drop n_init $ getLog)
                       
  mplot $ vector <$> [fromIntegral <$> [0 .. n_fft `div` 2 - 1],
                      fmap db10 . psdAbs . fmap ((!!3) . toList) . take n_fft . drop n_init $ log]
  putStrLn "fin."
